...
import { IReportDatagridProps } from './interfaces'
import { IReport, IReportParticipant, IReportPIC } from '../../interfaces'
import {
...
// Components
import ReportDatagridToolbar from '../report_datagrid_toolbar'

const ReportDatagrid = ({
  loading = false,
  rows,
}: // onExport,
IReportDatagridProps) => {
  // State
  const [pageSize, setPageSize] = React.useState<number>(PAGE_SIZE)
  const [selectionModel, setSelectionModel] = React.useState<GridSelectionModel>([])

  const columns: GridColDef<IReport>[] = [
    {
      field: 'date',
    ...
    },
    {
      ...
      flex: 3,
      minWidth: 200,
      valueFormatter: (params: GridValueFormatterParams<IReportPIC[]>) => {
    ...
    },
    {
      ...
      flex: 3,
      minWidth: 200,
      valueFormatter: (params: GridValueFormatterParams<IReportParticipant[]>) => {
        return params.value
          ? params.value
              .map(
                (v) =>
